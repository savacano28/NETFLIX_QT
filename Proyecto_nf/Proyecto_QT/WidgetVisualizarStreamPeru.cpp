...
WidgetVisualizarStreamPeru::WidgetVisualizarStreamPeru(int n, QWidget *parent) :
    QDialog(parent)
{
    widgetAgregarStreamPeru =new WidgetAgregarStreamPeru;
     ...
     connect(tabla,SIGNAL(cellClicked(int,int)),widgetAgregarStreamPeru,SLOT(ponerTabla(int,int)));
     connect(tabla,SIGNAL(cellClicked(int,int)),this,SLOT(armar()));

 }

 void WidgetVisualizarStreamPeru::armar(){

     QVector <string> x =widgetAgregarStreamPeru->devolverVec();
     int n=widgetAgregarStreamPeru->devolverSize();
