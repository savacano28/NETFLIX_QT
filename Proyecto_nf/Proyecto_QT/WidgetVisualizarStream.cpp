...
WidgetVisualizarStream::WidgetVisualizarStream(int n, QWidget *parent) :
    QDialog(parent)
{
    ...
    connect(tabla,SIGNAL(cellClicked(int,int)),widgetAgregarStream,SLOT(ponerTabla(int,int)));
    connect(tabla,SIGNAL(cellClicked(int,int)),this,SLOT(armar()));
    }

void WidgetVisualizarStream::armar(){

    QVector <string> x =widgetAgregarStream->devolverVec();

    int n=(widgetAgregarStream->devolverSize());
