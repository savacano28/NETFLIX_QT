...
WidgetVisualizarStreamEcuador::WidgetVisualizarStreamEcuador(int n, QWidget *parent) :
    QDialog(parent)
{
   widgetAgregarStreamEcuador =new WidgetAgregarStreamEcuador;
    ...
    connect(tabla,SIGNAL(cellClicked(int,int)),widgetAgregarStreamEcuador,SLOT(ponerTabla(int,int)));
    connect(tabla,SIGNAL(cellClicked(int,int)),this,SLOT(armar()));
  }

void WidgetVisualizarStreamEcuador::armar(){

    QVector <string> x =widgetAgregarStreamEcuador->devolverVec();
    int n=widgetAgregarStreamEcuador->devolverSize();
