...
QVector <string> BDscolombia;
QVector <int> rows;


WidgetAgregarStream::WidgetAgregarStream(QWidget *parent) :
    QDialog(parent)
{
...
void WidgetAgregarStream::agregarDatos(){
...
int WidgetAgregarStream::devolverSize(){
    return BDscolombia.size();
}

QVector <string> WidgetAgregarStream::devolverVec(){
    return BDscolombia;
}

void WidgetAgregarStream::ponerTabla(int row, int col){
    ...
    if(col==0){
        ...
        if(var==0){
            rows.append(row);
            BDscolombia.replace(pos,"1");
        }else{
            rows.remove(pos_r);
            BDscolombia.replace(pos,"0"); }
        }}
