...
QVector <string> BDvenezuela;

WidgetAgregarVenezuela::WidgetAgregarVenezuela(QWidget *parent) :
    QDialog(parent)
{
     ...
     QHBoxLayout *layoutRadial = new QHBoxLayout;
        ...
        connect(botonAceptar,SIGNAL(clicked()),this,SLOT(agregarDatos3()));
        connect (botonCancelar, SIGNAL(clicked()), this, SLOT(reject()));
}

void WidgetAgregarVenezuela::agregarDatos3(){
...
void WidgetAgregarVenezuela::otro(int pos,string data1){
     BDvenezuela.replace(pos,data1);
}


int WidgetAgregarVenezuela::devolverSize3(){
    return BDvenezuela.size();
}

QVector <string> WidgetAgregarVenezuela::devolverVec3(){
    return BDvenezuela;
}
